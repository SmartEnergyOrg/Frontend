...
export class WeatherService {
  ...
  constructor(private httpClient: HttpClient) {
    this.currentWeather = new BehaviorSubject<WeatherModel | undefined>(
      undefined
    );
    this.currentWeatherConfig = new BehaviorSubject<WeatherConfig | undefined>(
      undefined
    );
    ...
    this.getConfig().subscribe((config) => {
      ...
      this.currentWeatherConfig.subscribe((config) => {
        ...
        if (config) {
          this.connectOpenWeather(config?.lat!, config?.lon!).subscribe(
            (weather) => {
  ...
  connectWeatherPlace(city: string): Observable<WeatherConfig[] | any> {
    return this.httpClient.get<WeatherConfig[]>(
      `${this.apiUrl}/geo/1.0/direct?q=${city}&limit=20&appid=${this.apiKey}&lang=nl`
    );
  }
  ...
  connectOpenWeather(lat: number, lon: number): Observable<WeatherModel> {
    return this.httpClient.get<WeatherModel>(
      `${this.apiUrl}/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${this.apiKey}&units=metric`
    );
  }
  ...
  private repeatWeather(): void {
    ...
    interval(this.intervalTime).subscribe(() => {
      ...
      this.connectOpenWeather(
        this.currentWeatherConfig.value?.lat!,
        this.currentWeatherConfig.value?.lon!
  ...
  create(weatherConfig: WeatherConfig) {
    ...
    return this.httpClient
      .post<any>(this.SERVER_API_URL + '/api/weathers', weatherConfig)
      .pipe(
        map((value) => {
  ...
  update(weatherConfig: WeatherConfig) {
    ...
    return this.httpClient
      .put<any>(this.SERVER_API_URL + '/api/weathers', weatherConfig)
      .pipe(
        map((value) => {
          if (value.status != 401) {
            const newConfig = value.result as WeatherConfig;
